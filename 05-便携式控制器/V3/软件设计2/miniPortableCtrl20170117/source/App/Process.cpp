...
{	
	...
	ledTest       = new IoOut(LED_TEST_PORT,LED_TEST_PIN);//低电平有效
	ledKey        = new IoOut(LED_KEY_PORT,LED_KEY_PIN);
...
{	
	...
	ADKey1Value  = AD_Filter[AD_NUM_KEY1]*42*3>>12;
	ADKey2Value  = AD_Filter[AD_NUM_KEY2]*42*3>>12;
	ADKey3Value  = AD_Filter[AD_NUM_KEY3]*42*3>>12;
...
{	
	//按键1 ---全速/休眠唤醒 模式切换
	static u8 preADKey1Value=0;
	
	
	if(preADKey1Value!=ADKey1Value)
	{		
		preADKey1Value=ADKey1Value;	
		if(ADKey1Value>5)
		{				
			isSleepMode=1-isSleepMode;
		}
	}
	else
	{
