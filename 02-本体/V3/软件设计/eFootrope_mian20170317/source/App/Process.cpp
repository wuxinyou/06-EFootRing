...
void Process::runOnTime3(void)
{
	...
	{		
		...
		//用于 没有收到数据后，控制电击时间
		else if(overTimerCount>=OVER_TIME*2)
		{		
			overTimerCount=0;
		...
		//	if(preControlStatus!=controlStatus)
			{
				...
				{
					greenLed->setOn();//测试20170317
				//		shockTime=0;    //按下电击，开始计时电击时间
								//定时关闭电击，放置长期电击
					shockTime++;
					if(shockTime>CONTINUE_SHOCK_TIME)    //
					{
						//shockTime会在resetControlData里复位
						shockPowCtrl->setOff();
						shockCtrl->setOff();	
						resetControlData();   //电击完成后，需要复位控制指令
						readySleep();    //准备休眠
...
/**
  ...
  * @retval None
  */
void Process::readySleep()
{
	watchDogTimer=50;    //触发休眠
}
