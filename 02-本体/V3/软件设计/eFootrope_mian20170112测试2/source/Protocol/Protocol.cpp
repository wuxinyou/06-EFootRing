...
//	com1st初始化
void Protocol::com1st_Init(void)
{
...
//	com1st更新数据
void Protocol::updateSendBuf(uint8_t* tempData)
{
	com1st_SendBuf[0]=COMN1_SEND_HEAD;
	com1st_SendBuf[1]=COMN1_SEND_ID;
...
//	com1st发送数据
void Protocol::com1st_Send(void)
{
	com1st->write(com1st_SendBuf,COMN1_SEND_LENGTH);
}

//	com1st接收数据
bool Protocol::com1st_Receive(void)
{
	uint8_t checkSum=0;
	bool readFlag=false;
	...
	while (com1st->readAllow()&&(readFlag==false))
	{
		...
		if(com1st_ReceiveChain->getData()==COMN1_RECEIVE_HEAD)		//	检查帧头
		{
			...
			com1st_ReceiveChain->next();
			if(com1st_ReceiveChain->getData()==COMN1_RECEIVE_ID)	//	检查ID
			{
...
//	获取com1st接收到的数据
uint8_t* Protocol::com1st_GetData(void)
{
	return	com1st_ReceiveBuf;
}

//	com2nd初始化
void Protocol::com2nd_Init(void)
{
...
void Protocol::com2nd_UpdateSendBuf(uint8_t* tempData)
{
	...
	com2nd_SendBuf[1]=COMN2_SEND_ID;
...
//	com2nd_发送数据
void Protocol::com2nd_Send(void)
{
	com2nd->write(com2nd_SendBuf,COMN2_SEND_LENGTH);
}

//	com2nd_接收数据
bool Protocol::com2nd_Receive(void)
{
	uint8_t checkSum=0;
	bool readFlag=false;
	...
	while (com2nd->readAllow()&&(readFlag==false))
	{
		...
		if(com2nd_ReceiveChain->getData()==COMN2_RECEIVE_HEAD)		//	检查帧头
		{
			...
			com2nd_ReceiveChain->next();
			if(com2nd_ReceiveChain->getData()==COMN2_RECEIVE_ID)	//	检查ID
			{
...
//	获取com2nd_接收到的数据
uint8_t* Protocol::com2nd_GetData(void)
{
