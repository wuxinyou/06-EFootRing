...
#ifdef USARTREMAP_3_TO_PORTD	//串口重映射到PORTC上
...
void SerialPort::usartGpioInit()
{
	...
	switch(temPort)
	{
...
#endif
			STM32_GpioOneInit(USARTPIN_IN_1, GPIO_Speed_50MHz, GPIO_Mode_IN_FLOATING,USARTPORT_1);	//浮空输入（也可以设置成带上拉输菺PIO_Mode_IPU耄?
...
#endif
			STM32_GpioOneInit(USARTPIN_IN_2, GPIO_Speed_50MHz, GPIO_Mode_IN_FLOATING,USARTPORT_2);	//浮空输入（也可以设置成带上拉输菺PIO_Mode_IPU耄?
		...
		case 3:
			...
			STM32_GpioOneInit(USARTPIN_IN_3, GPIO_Speed_50MHz, GPIO_Mode_IN_FLOATING,USARTPORT_3);	//浮空输入（也可以设置成带上拉输菺PIO_Mode_IPU耄?
		...
		case 4:
			...
			STM32_GpioOneInit(USARTPIN_IN_4, GPIO_Speed_50MHz, GPIO_Mode_IN_FLOATING,USARTPORT_4);	//浮空输入（也可以设置成带上拉输菺PIO_Mode_IPU耄?
		...
		case 5:
			...
			STM32_GpioOneInit(USARTPIN_IN_5, GPIO_Speed_50MHz, GPIO_Mode_IN_FLOATING,USARTPORT_5_IN);	//浮空输入（也可以设置成带上拉输菺PIO_Mode_IPU耄?
...
*输入框中填写相应的CPU类型宏，否则会出现中断进不去的情况
*串口发送完成的中断不必要打开，该中断在多缓存发送时才有必要使用
*USART_IT_TXE标志清零是硬件自动清零的，老程序有错误，打开断言，USART_ClearITPendingBit(USART3,USART_IT_TXE)，通不过
********************************************************************************************/
