...
/* 类型定义 --------------------------------------------------------------------*/
typedef int16_t(*setSpeedFun)(int16_t);
/* 宏定义-----------------------------------------------------------------------*/ 
//U0指令发送的临界时间
...
public:
	...
	void setSpeedFromSubAngle(int,int,int);			//根据角度差设置速度

	void setSpeed(int16_t);
	void setPWMSpeed(int16_t value);//{feedPWMSpeed=value; uspeedTime=UO_TIME_MAX;}
...
// 						{feedSpeed=(direction?referSpeedMax:(-referSpeedMax));}
  ...
  void setSpeedFromSubAngle(setSpeedFun,int,int,int);	//根据角度差设置速度
protected:
