...
//运行到指定位置的角度误差范围（±0.5度）
#define ANGLE_ALLOWANCE 	10

//减速临界位

#define ANGLE_SLOW_START	1000
#define ANGLE_SLOW_GO			400
#define ANGLE_SLOW_DOWN		150
...
void MotorControl::setSpeed(int16_t value)
{
	...
	else if(temValue<0&&temValue>-100)
	{
		temValue=-100;
	}
	feedSpeed=temValue;
...
/**
  ...
  * 
  */
void MotorControl::setPWMSpeed(int16_t value)
{
	int16_t temValue=value;
	if(temValue>0&&temValue<100)
	{
...
/**
  ...
  * 
  */
void MotorControl::setSpeedFromSubAngle(int subA,int sunT,int sunMaxT)
{
	if(ABS(subA)>ANGLE_ALLOWANCE)//误差在 ±0.5 度
	{
		
		if(ABS(subA)>ANGLE_SLOW_START)
			setSpeed((int16_t)(((int)referSpeedMax)*sunT/sunMaxT));
		else if(ABS(subA)>ANGLE_SLOW_GO)
			setSpeed((int16_t)(((int)referSpeedMin)*10*sunT/sunMaxT));
		else if(ABS(subA)>ANGLE_SLOW_DOWN)
			setSpeed((int16_t)(((int)referSpeedMin)*6*sunT/sunMaxT));
		else
			setSpeed((int16_t)(((int)referSpeedMin)*sunT/sunMaxT));
	}
	else								//速度置0
	{
		resetSpeed();
...
/**
  ...
  * 
  */
void MotorControl::setSpeedFromSubAngle(setSpeedFun fun,int subA,int sunT,int sunMaxT)	//根据角度差设置速度
{
	if(ABS(subA)>ANGLE_ALLOWANCE)//误差在 ±0.5 度
	{
		if(ABS(subA)>ANGLE_SLOW_START)
			fun((int16_t)(((int)referSpeedMax)*sunT/sunMaxT));
		else if(ABS(subA)>ANGLE_SLOW_GO)
			fun((int16_t)(((int)referSpeedMin)*10*sunT/sunMaxT));
		else if(ABS(subA)>ANGLE_SLOW_DOWN)
			fun((int16_t)(((int)referSpeedMin)*6*sunT/sunMaxT));
		else
			fun((int16_t)(((int)referSpeedMin)*sunT/sunMaxT));
	}
	else								//速度置0
	{
		resetSpeed();
...
/**
  ...
  * 
  */
void MotorControl::updateUspeedTime()										//更新U指令时间 保证只有V=0时U才发送
{
...
{
	if(uspeedTime<UO_TIME_MAX)//发送V V0最多发送0.5秒 自动转U模式发送
		sendSpeedCommand(0x01);
	else
		sendSpeedCommand(0x03);//发送U
}
