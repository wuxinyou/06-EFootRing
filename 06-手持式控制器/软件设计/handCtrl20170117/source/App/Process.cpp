...
#include <stdlib.h>
 ...
 const u8 Coordinate_X[11]={192,66,80,94,108,122,136,150,164,178,206};  //数码显示液晶屏X坐标数组
/* 类的实现--------------------------------------------------------------------*/
	...
	//按一下Num1键
 	if (preNumdKey[1] != keyNum1->FinalState)					//	
 	{
		...
		if (keyNum1->FinalState==false)
		{
			...
			{
				sqStack->Push(1);			
				showNumber(Coordinate_X[sqStack->GetTop()],sqStack->GetTopElem() );
			}
 	...
 	if (preNumdKey[10] != keyDel->FinalState)					//	
 	{
		...
		if (keyDel->FinalState==false)
		{
			...
			{
				showNumber(Coordinate_X[sqStack->GetTop()],10 );		//显示空白背景，代替清除		
				sqStack->Pop();
...
void Process::showInputWin(bool flag)
{
	if(flag)
	{
		LCDControl->CutPicAndShow(2,80,2,15,15,300,1);    //显示输入编码框
	}
	else
	{
		LCDControl->CutPicAndShow(3,80,2,15,15,300,1);    //显示背景色
	}
...
void Process::showNumber(int position,int num)
{
	...
	if(num>9)
	{
		LCDControl->CutPicAndShow(3,Coordinate_X[10],95,14,24,Coordinate_X[position],141);    //显示空白背景
	}
	else
	{
		LCDControl->CutPicAndShow(3,Coordinate_X[num],95,14,24,Coordinate_X[position],141);    //显示输入编码
	}
...
void Process::showPower()
{
	//显示电量图标
	if(powerValue>120)
		LCDControl->CutPicAndShow(2,10,15,60,30,188,102);    //显示100
	else if(powerValue>115)
		LCDControl->CutPicAndShow(2,10,57,60,30,188,102);    //显示80
	else if(powerValue>113)
		LCDControl->CutPicAndShow(2,10,102,60,30,188,102);    //显示60
	else if(powerValue>111)
		LCDControl->CutPicAndShow(2,10,147,60,30,188,102);    //显示40
	else 
		LCDControl->CutPicAndShow(2,10,191,60,30,188,102);    //显示20
...
void Process::showControlMode()
{
	...
	{
		LCDControl->CutPicAndShow(2,219,61,60,30,188,59);    //显示 接近控制
	//	showInputWin(false);
	}
	//定点控制
	else if(0==controlModeFlag)
	{
		LCDControl->CutPicAndShow(2,119,61,60,30,188,59);    //显示 定点控制
		//showInputWin(true);
	}
...
{
	//更新数据
	sendData[0]=(controlStatusFlag <<2)+controlModeFlag;
