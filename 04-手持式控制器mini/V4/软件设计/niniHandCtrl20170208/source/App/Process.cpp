...
void Process::showControlStatus()
{
	...
	if(2==controlStatusFlag)     //正在电击
	{
		...
		//显示电击
		LCDCtrl->writeHanziLine(6,90,"电击",2,true,0,0x00);
	...
	else if(1==controlStatusFlag) //正在警报
	{
		 ...
		 //显示警报
		LCDCtrl->writeHanziLine(6,90,"警报",2);
...
{
	LCDCtrl->clearScreen(0X00);//清屏
	LCDCtrl->writeHanziLine(0,4,"☆☆★欢迎光临★",8,true,0,0x00);
	LCDCtrl->writeHanziLine(2,0,"☆排爆机器人磨机系统☆",11,true,0,0x00);
...
{
	LCDCtrl->writeHanziLine(0,22,"电子脚扣控制器",7,true,0,0x00);
	
	//显示电量提示：
	LCDCtrl->writeHanziLine(2,4,"电量：",3,true,0,0x00);
