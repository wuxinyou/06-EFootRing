...
void Process::runOnTime2(void)
{
	...
	if(index>0)    //当有操作按键（电击，警报）触发后，index开始++
		index++;
	...
	if( returnKeyEvent() )
	{
		showControlStatus();
		showControlMode();
		index=1;       //一旦有操作按键后，触发index++
	}

	//如果没有按键按下，复位控制数据
	if(index%20==0)       //index自加到20后，
	if( !returnKeyValue() )
	{
	//	controlStatusFlag=0;  //测试时，禁止归零
		index=0;
	}
...
{	
	...
	if(sendCounter>0)   //为了保证上电不会自动++,只有按下按键后，触发sendCounter++
		sendCounter++;
	if(sendCounter>10)  //发送完成10次后，自动归零
		sendCounter=0;
	
	if(controlStatusFlag)
	{
		sendCounter=1;      //按下按键后，触发sendCounter++
		updataToZigbee();
	}
	if(sendCounter<10 && sendCounter>0)
	{
	//	updataToZigbee();   //在sendCounter++中连续发送9次
	}
...
void Process::runOnTime4(void)
{
	...
	else if(testTimeCount>60)
	{
		controlStatusFlag=2;   //触发电击 电击5个周期，直到testTimeCount=50
	}
	//add done
...
{	
	...
	//按键实例化
	keyNum1    = new Key(KEYNUM1_PORT,KEYNUM1_PIN,true);    //数字按键1
	keyNum2    = new Key(KEYNUM2_PORT,KEYNUM2_PIN,true);     //数字按键2
	keyNum3    = new Key(KEYNUM3_PORT,KEYNUM3_PIN,true);    //数字按键3
	keyNum4    = new Key(KEYNUM4_PORT,KEYNUM4_PIN,true);    //数字按键4
	keyNum5    = new Key(KEYNUM5_PORT,KEYNUM5_PIN,true);    //数字按键5
	keyNum6    = new Key(KEYNUM6_PORT,KEYNUM6_PIN,true);    //数字按键6
	keyNum7    = new Key(KEYNUM7_PORT,KEYNUM7_PIN,true);    //数字按键7
	keyNum8    = new Key(KEYNUM8_PORT,KEYNUM8_PIN,true);    //数字按键8
	keyNum9    = new Key(KEYNUM9_PORT,KEYNUM9_PIN,true);    //数字按键9
	keyNum0    = new Key(KEYNUM0_PORT,KEYNUM0_PIN,true);    //数字按键0

	key_NC     = new Key(KEYNC_PORT,KEYNC_PIN,true);      //备用按键
	keyDel     = new Key(KEYDEL_PORT,KEYDEL_PIN,true);      //退格键
	keyMode    = new Key(KEYMODE_PORT,KEYMODE_PIN,true);   //模式按键
	keyShock   = new Key(KEYSHOCK_PORT,KEYSHOCK_PIN,true);
	keyAlarm   = new Key(KEYALARM_PORT,KEYALARM_PIN,true);
...
{
 	static 	bool	preNumdKey[15]		={false};
	...
	//按一下Num1键
 	...
 	if (preNumdKey[11] != keyMode->FinalState)					//	
 	{
		...
		if (keyMode->FinalState==false)
		{
			...
			//按一下Key_NC键
 	if (preNumdKey[14] != key_NC->FinalState)					//	
 	{
 		preNumdKey[14]=key_NC->FinalState;
		if (key_NC->FinalState==false)
		{
			Pow_EN->setOff();	//按下Key_NC键后，关闭电源
