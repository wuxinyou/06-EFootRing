...
//显示汉字
void OLED_ShowEFootCHinese(u8 x,u8 y,u8 no)
{      			    
	u8 t,adder=0;
	OLED_Set_Pos(x,y);	
    for(t=0;t<16;t++)
		{
				OLED_WR_Byte(eFoot[2*no][t],OLED_DATA);
				adder+=1;
     }	
		OLED_Set_Pos(x,y+1);	
    for(t=0;t<16;t++)
			{	
				OLED_WR_Byte(eFoot[2*no+1][t],OLED_DATA);
				adder+=1;
      }					
}

//显示汉字
void OLED_ShowEFootCHinese12X12(u8 x,u8 y,u8 no)
{      			    
	u8 t,adder=0;
	OLED_Set_Pos(x,y);	
    for(t=0;t<12;t++)
		{
				OLED_WR_Byte(eFoot12[2*no][t],OLED_DATA);
				adder+=1;
     }	
		OLED_Set_Pos(x,y+1);	
    for(t=0;t<12;t++)
			{	
				OLED_WR_Byte(eFoot12[2*no+1][t],OLED_DATA);
				adder+=1;
      }					
}
...
{ 	
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_4|GPIO_Pin_5|GPIO_Pin_7|GPIO_Pin_3;	 //PD3,PD6推挽输出  
 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		 //推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;//速度50MHz
 	GPIO_Init(GPIOA, &GPIO_InitStructure);	  //初始化GPIOD3,6
 	GPIO_SetBits(GPIOA,GPIO_Pin_4|GPIO_Pin_5|GPIO_Pin_7|GPIO_Pin_3);	//PD3,PD6 输出高
  ...
  OLED_RST_Set();
	Delay_1ms(1000);   //这里的延时很讲究，由于液晶屏电路有电容器件，需要一些时间先给硬件上电，然后才能发送指令
	OLED_RST_Clr();
	Delay_1ms(1000);   //这里的延时很讲究，由于液晶屏电路有电容器件，需要一些时间先给硬件上电，然后才能发送指令
	OLED_RST_Set(); 
