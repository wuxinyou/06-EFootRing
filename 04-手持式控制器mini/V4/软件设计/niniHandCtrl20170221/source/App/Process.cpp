...
/* 类的实现--------------------------------------------------------------------*/
 	const u8 Coordinate_X[15]={/*0,8,16,32,40,*//*48,56,64,72,80,88,96,104,112,120*/36,44,52,96,104}; //液晶屏X 坐标数组
...
{	
	...
	keyCheck = new Key(KEYCHECK_PORT,KEYCHECK_PIN);	//查询本体电量
...
void Process::getPowerValue(void)
{
	...
	powerValue  = AD_Filter[NUM_POWER]*33*3>>8;
...
{
	static bool preKeyStatus[4] = {true};
	...
	if(preKeyStatus[3] != keyCheck->FinalState)
	{
		preKeyStatus[3] = keyCheck->FinalState;
		return true;
	}
	return false;
}
...
bool Process::returnKeyValue()
{
	...
	if(!keyCheck->FinalState ||!keyMode->FinalState || !keyAlarm->FinalState || !keyShock->FinalState || !keyDel->FinalState
...
{
 	static 	bool	preNumdKey[15]		={true};
 	...
 	if (preNumdKey[13] != keyAlarm->FinalState)					//	
 	{
		...
		if (keyAlarm->FinalState==false)
		{
			...
			showControlStatus();
				...
				//按下CHECK键
	if (preNumdKey[14] !=keyCheck->FinalState)
	{
		preNumdKey[14]=keyCheck->FinalState;
		if(keyCheck->FinalState==false)
		{
			checkPowerMode=1;			// 查询本体电量
			controlModeFlag=1;
...
*/
	...
	u16 Value=592;											//扩大电压的取值范围
	int tmp=(powerValue-Value)/4*5;						//设置区间为0-100
	if(tmp <0)
	tmp = -tmp;
	u8 str[5];														
	itoa(tmp,str);
	OLED_ShowString(36,2,str);
	
	OLED_ShowChar(76,2,'%');						//在固定位置显示%	
...
void Process::showControlMode()
{
	...
	//  LCDCtrl->writeHanziLine(6,4,"定点控制",4,true,0,0x00);
		...
		//显示输入群组：
		OLED_ShowEFootCHinese12X12(0,4,22);
		OLED_ShowEFootCHinese12X12(12,4,23);
		OLED_ShowEFootCHinese12X12(24,4,19);
		
		
		//显示输入编码：
		OLED_ShowEFootCHinese12X12(60,4,17);
		OLED_ShowEFootCHinese12X12(72,4,18);
		OLED_ShowEFootCHinese12X12(84,4,19);
...
/**显示电量不足的编号
	...
	*
**/	
void Process::showNoPower()
{
	if(checkPowerMode)
	{
		OLED_Clear();
		
		OLED_ShowEFootCHinese12X12(34,0,24);
		OLED_ShowEFootCHinese12X12(46,0,20);
		OLED_ShowEFootCHinese12X12(58,0,17);
		OLED_ShowEFootCHinese12X12(70,0,18);
		OLED_ShowEFootCHinese12X12(82,0,19);
		
// 		receiveData[0] = zigbeeControl->reciveBuf[4]+'0';
// 		receiveData[1] = zigbeeControl->reciveBuf[5];
// 		receiveData[2] = zigbeeControl->reciveBuf[6];
// 		receiveData[3] = zigbeeControl->reciveBuf[7];
// 		receiveData[4] = zigbeeControl->reciveBuf[8];
		
		
		itoa(Data,receiveData);
		for(int j=1; j<8;j++)
		{
		  for(int i=60; i<=120; i+=8)
			{				
				OLED_ShowString(i,j,receiveData);
...
{
	//更新数据
	sendData[0]=(checkPowerMode<<5) + (isFreeMode<<4) +(controlStatusFlag <<2)+controlModeFlag;
